...
public class Cliente extends Pessoa implements InterfaceCliente{
	...
	public boolean obterEmprestimo(int valor) {
		...
		if ((valor + this.valorDaDivida) > dividaMaxima) return (false);
		else {
			this.valorDaDivida += valor;
			this.valorContaCorrente += valor;
			return (true);
		}
	}
	...
	public boolean pagarEmprestimo(int valor) {
		if (valor <= 0) return (false);
		if (valor > this.valorDaDivida || valor > this.valorContaCorrente) return (false);
		else {
			this.valorDaDivida -= valor;
			this.valorContaCorrente -= valor;
			return (true);
	...
	public boolean negativado() {
		if (this.valorContaCorrente < this.valorDaDivida) return (true);
		return (false);
	}
	...
	public boolean realizarSaque(int valor) {
		if (valor <= 0) return (false);
		if (valor > this.valorContaCorrente) return (false);
		else {
			this.valorContaCorrente -= valor;
			return (true);
		}
